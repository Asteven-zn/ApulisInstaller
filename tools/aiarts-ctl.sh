...
function help-info() {
    case "$1" in
        (add-etcd)
            echo -e "Usage: aiarts add-etcd <new_etcd_ip>\n\nread 'https://github.com/easzlab/kubeasz/blob/master/docs/op/op-etcd.md'"
            ;;
        (add-master)
            echo -e "Usage: aiarts add-master <new_master_ip>\n\nread 'https://github.com/easzlab/kubeasz/blob/master/docs/op/op-master.md'"
            ;;
        (add-node)
            echo -e "Usage: aiarts add-node <new_node_ip>\n\nread 'https://github.com/easzlab/kubeasz/blob/master/docs/op/op-node.md'"
            ;;
        (del-etcd)
            echo -e "Usage: aiarts del-etcd <etcd_ip>\n\nread 'https://github.com/easzlab/kubeasz/blob/master/docs/op/op-etcd.md'"
            ;;
        (del-master)
            echo -e "Usage: aiarts del-master <master_ip>\n\nread 'https://github.com/easzlab/kubeasz/blob/master/docs/op/op-master.md'"
            ;;
        (del-node)
            echo -e "Usage: aiarts del-node <node_ip>\n\nread 'https://github.com/easzlab/kubeasz/blob/master/docs/op/op-node.md'"
            ;;
        (basic-auth)
            echo -e "Usage: aiarts basic-auth <options>\nOption:\t -s enable basic-auth\n\t -S disable basic-auth\n\t -u <user> set username\n\t -p <pass> set password"
            ;;
        (*)
            usage
            return 0
...
function add-node() {
    ...
    sed -i "/\[kube-worker/a $1 NEW_NODE=yes ${@:2}" $BASEPATH/hosts
...
function add-master() {
    ...
    sed -n '/^\[kube-master/,/^\[kube-worker/p' $BASEPATH/hosts|grep "^$1[^0-9]*$" && { echo "[ERROR] master $1 already existed!"; return 2; }
    ...
    ansible-playbook $BASEPATH/05.kube-worker.yml -t restart_lb -e MASTER_CHG=yes || { echo "[ERROR] Failed to restart the haproxy service on 'kube-worker' nodes!"; return 2; }
...
function del-node() {
    ...
    # 
    ansible-playbook $BASEPATH/tools/12.delnode.yml -e NODE_TO_DEL=$1 || { echo "[ERROR] Failed to delete 'kube-worker': $1!"; return 2; }
...
function setup() {
    [ -f "$BASEPATH/.cluster/current_cluster" ] || { echo "[ERROR] invalid context, run 'aiarts checkout <cluster_name>' first"; return 1; }
...
function start-aio(){
    ...
    if [[ -z $HOST_IP ]];then
        # aiarts runs in a host machine, get host's ip
